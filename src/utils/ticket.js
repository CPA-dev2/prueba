import { toInteger } from "lodash";

const get_string_full_date = (_date)  => {
    let date = new Date(_date);
    let day = date.getDate() <= 9 ? "0" + date.getDate() : date.getDate();
    let month =
      date.getMonth() <= 9 ? "0" + (date.getMonth() + 1) : date.getMonth() + 1;
    return day + "-" + month + "-" + date.getFullYear() + ' ' +  date.getHours() + ':' + date.getMinutes();
  };
  
  const get_html = (data) => {
   
    total = data.datos_pago.cuota + data.datos_pago.mora
    
    const logo = '<img style="display: block; margin-left: auto; margin-right: auto;"' +
    'width="100px" src="data:image/png;base64," />';

    const header = '<div><table style="margin-bottom: 1px; margin-top: 1px; width: 100%; padding: 0" ><tbody>' +
    '<tr>' +
    '<td style="font-weight: bold; text-align: left; width: 60%; border-bottom-style: dashed; border-bottom-width: 1px; border-bottom-color: black; " >TICKET DE OPERACIÓN </td>' +
    '<td style=" color: gray; width: 40%; text-align: right; border-bottom-style: dashed; border-bottom-width: 1px; border-bottom-color: black; " >' + get_string_full_date(new Date()) + '</td>' +
    '</tr>' +
    '<tr>' +
    '<td style="color: gray; text-align: left" colspan="2">No. <span style="font-weight: bold; text-align: left; color: black"><br />' + data.numero_ticket + '</span ></td>' +
    '</tr>' +
    '<tr>' +
    '<td style="color: gray; text-align: left" colspan="2">Transacción <span style="font-weight: bold; text-align: left; color: black"><br />ABONO</span ></td>' +
    '</tr>' +         
    '<tr><td style="color: gray; text-align: left" colspan="2">Asesor: <span style="font-weight: bold; text-align: left; color: black"><br />' + data.gestor.toUpperCase() + '</span ></td>' +
    '</tr>' +
    '</tbody></table></div>' 
    
    const _data = '<div>' +
    '<table style="margin-bottom: 1px; margin-top: 1px; width: 100%; padding: 0" >' +
    '<tbody>' +
    '<tr>' +
    '<td style=" color: gray; text-align: left; border-bottom-style: dashed; border-bottom-width: 1px; border-bottom-color: black; border-top-style: solid; border-top-width: 1px; border-top-color: black; " colspan="2" >' +
    '<br />DESCRIPCIÓN <br />' +
    '<span style="font-weight: bold; text-align: left; color: black" >Abono de cuota de Q' + toInteger(total).toFixed(2) + ' al cliente ' + data.cliente.toUpperCase() + '</span> <br /> ' +
    '</td>' +
    '</tr>' +
    '<tr>' +
    '<td style=" font-weight: bold; text-align: left; width: 60%; border-bottom-style: dashed; border-bottom-width: 1px; border-bottom-color: black; " >' +
    'MONTO' +
    '</td>' +
    '<td style=" color: black; width: 40%; text-align: right; border-bottom-style: dashed; border-bottom-width: 1px; border-bottom-color: black; " >' +
    'Q' + toInteger(total).toFixed(2) +
    '</td>' +
    '</tr>' +
    '</tbody>' +
    '</table>' +
    '</div>'

    const footer = '<div>' +
    '<div style="width: 100%; text-align: center; font-weight: bold;">' +
    '<p>IMPORTANTE</p>' +
    '</div>' +
    '<div style="text-align: center; margin-bottom: 100px;">' +
    '<p>' +
    'Empeña con nosotros y recibe tu regalo sorpresa, recuerda que en ' +
    'PRENDA CREDITO AVANZA empeñar es mejor! Con Prenda Crédito Avanza ' +
    'sales ganando. Solicita tu avalúo a domicilio llamando al 4120-9166.' +
    '<br />www.prendacreditoavanza.gt<br />Whatsapp 4120-9166' +
    '</p>' +
    '</div>' +
    '</div>'

    const styles_print = '<style>@page{ margin-left: 3pt; margin-right: 3pt; margin-top: 3pt; margin-bottom: 3pt; padding-left: 0pt; padding-right: 0pt; padding-top: 0pt; padding-bottom: 0pt;} </style>'
    const body = '<body style="font-size: 5px !important;">' + 
        styles_print +
        logo +
        header +
        _data +
        footer +
        '</body>'
    const html = '<html>' + body + '</html>';
      return html.repeat(2);
    };
  
  export const ticket = {
      get_html,
  };
  