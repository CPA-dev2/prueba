
const get_string_full_date = (_date)  => {
  let date = new Date(_date);
  let day = date.getDate() <= 9 ? "0" + date.getDate() : date.getDate();
  let month =
    date.getMonth() <= 9 ? "0" + (date.getMonth() + 1) : date.getMonth() + 1;
  return day + "-" + month + "-" + date.getFullYear() + ' ' +  date.getHours() + ':' + date.getMinutes();
};

const get_html = (data, solicitud, nuevo_plan) => {
  const primary_color = 'black';
  const secondary_color = 'gray';

  let rows = '';
    data.forEach((item) => {
      rows = rows +
      '<tr style="text-align:center;">' +
        '<td>' + item.numero + '</td>' +
        '<td>' + item.fecha + '</td>' +
        '<td style="text-align:right;"> Q' + item.cuota.toFixed(2) + '</td>' +
        '<td style="text-align:right;"> Q' + item.saldo.toFixed(2) + '</td>' +
      '</tr>';
    });

    const data_table = '<div><table style="font-size: 5px; width:100%;"><thead><tr style="text-align:center;font-weight: bold; width: 100%;"><td style="width: 15%;">No.</td><td style="width: 30%;">Fecha</td><td style="width: 30%; text-align:right;">Cuota</td><td style="width: 25%; text-align:right;">Saldo</td></tr></thead><tbody>' + rows + '</tbody></table></div><div>';
    const title_table = '<div>'  +
      '<table style="font-size: 5px; margin-bottom: 1px; margin-top: 1px; width: 100%; padding: 0;">' +
        '<tbody>' +
        '<tr>' +
          '<td style="font-weight: bold; text-align: left; width: 60%; border-bottom-style: dashed; border-bottom-width: 1px; border-bottom-color: black;">CONVENIO DE COBRANZA DIFÍCIL</td>' +
          '<td style="color: ' + secondary_color + '; width: 40%; text-align: right; border-bottom-style: dashed; border-bottom-width: 1px; border-bottom-color: black;">' + get_string_full_date(new Date())  + '</td>' +
        '</tr>' +
        '<tr>' +
          '<td style="color: ' + secondary_color + '; text-align: left;" colspan="2">' + 'Cliente: ' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';">' + solicitud.cliente?.nombres.toUpperCase() + ' '  + solicitud.cliente?.apellidos.toUpperCase() + '</span>' + '</td>' +
        '</tr>' +
        '<tr>' +
          '<td style="color: ' + secondary_color + '; text-align: left; border-bottom-style: solid; border-bottom-width: 1px; border-bottom-color: black;" colspan="2">' + 'Tel.: ' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';">' + solicitud.cliente.telefono + '</span>' + '</td>' +
        '</tr>' +
        '<tr>' +
          '<td style="color: ' + secondary_color + '; text-align: left;">Plan</td>' +
          '<td style="color: ' + secondary_color + '; text-align: left;">Monto</td>' +
        '</tr>' +
        '<tr>' +
        '<td style="font-weight: bold; text-align: left; font-size: 12px;">' + nuevo_plan.nombre + '</td>' +
        '<td style="font-weight: bold; text-align: left; font-size: 12px;">Q' + (solicitud.saldo_pendiente * (1 + nuevo_plan.interes)).toFixed(2) + '</td>' +
        '</tr>' +
        '</tbody>' +
      '</table></div>';

    const logo = '<img style="display: block; margin-left: auto; margin-right: auto;"' +
      'width="100px" src="data:image/png;base64," />';
    const _page_styles = '<style> @page{ background-color: red; margin-left: 3pt; margin-right: 3pt; margin-top: 3pt; margin-bottom: 3pt; padding-left: 0pt; padding-right: 0pt; padding-top: 0pt; padding-bottom: 0pt; } </style>';
    const footer = '<div style="width: 100%;  text-align: justify;">' +
      '<p style="font-size:5px;"> *HORARIO DE PAGO: De 08:00 a 16:00 Cualquier pago reportado fuera del horario se cobrará mora de Q.25.00*</p></div>' +
      '<div style="text-align:center;"><p style="font-size:5px;"> *Acepto y entiendo las condiciones de pago</p><div>' +
      '<div style="text-align:center;margin-top:25px;"><p style="font-size:5px;"> F._______________________________________</p><div>';
    const body =
    '<body>' +
        _page_styles +
        logo +
        title_table +
        data_table +
        footer +
    '</body>';
    const html = '<html>' + body + '</html>';
    return html;
  };

export const html_utils = {
    get_html,
};
