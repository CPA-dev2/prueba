const get_string_full_date = (_date)  => {
    let date = _date ? new Date(_date) : new Date();
    let day = date.getDate() <= 9 ? "0" + date.getDate() : date.getDate();
    let month =
      date.getMonth() <= 9 ? "0" + (date.getMonth() + 1) : date.getMonth() + 1;
    return day + "-" + month + "-" + date.getFullYear() + ' ' +  (date.getHours() <= 9 ? ('0' + date.getHours()) : date.getHours() ) + ':' + (date.getMinutes() <= 9 ? '0' + date.getMinutes() : date.getMinutes() );
  };

const get_html = (data, gestor, dia) => {
    const primary_color = 'black';
    const secondary_color = 'gray';
    
    let _data = "";
    data.forEach((solicitud) => {
          const fechas = solicitud.fechas_pago[1];
          const pactado = fechas.capital + fechas.interes;

        _data =  _data + '<div>'  +
        '<table style="font-size: 5px; margin-top: 10px; margin-bottom: 10px; width: 100%; padding: 0; border-bottom-style: dashed; border-bottom-width: 1px; border-bottom-color: ' + primary_color + ';">' +
          '<tbody>' +
          '<tr>' +
            '<td style="width: 50%;"></td>' +
            '<td style="width: 50%;"></td>' +
          '</tr>' +
          '<tr>' +
            '<td style="color: ' + secondary_color + '; text-align: left;" colspan="2"><span style="font-weight: bold; text-align: left; color: ' + primary_color + ';">' + solicitud.cliente?.nombres.toUpperCase() + ' '  + solicitud.cliente?.apellidos.toUpperCase() + '</span>' + '</td>' +
          '</tr>' +
          '<tr>' +
            '<td style="color: ' + secondary_color + '; text-align: left;" colspan="2">' + solicitud.plan.periodo + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';">' + '</span>' + '</td>' +
          '</tr>' +
          '<tr>' +
            '<td style="color: ' + secondary_color + '; text-align: left;">' + 'Monto: ' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';"> Q' + (solicitud.saldo_pendiente !== null ? solicitud.saldo_pendiente.toFixed(2) : '0.00')  + '</span>' + '</td>' +
            '<td style="color: ' + secondary_color + '; text-align: left;">' + 'Pactado: ' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';"> Q' + pactado.toFixed(2) + '</span>' + '</td>' +
          '</tr>' +
          '<tr>' +
            '<td style="color: ' + secondary_color + '; text-align: left;">' + 'Mora: ' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';"> Q' +  (solicitud.monto_mora_pendiente !== null ? solicitud.monto_mora_pendiente.toFixed(2) : '0.00') + '</span>' + '</td>' +
            '<td style="color: ' + secondary_color + '; text-align: left;">' + 'Del día: ' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';"> Q' + solicitud.pendiente[dia].toFixed(2) + '</span>' + '</td>' +
          '</tr>' +
          '</tbody>' +
        '</table></div>';
    });
    const title_table = '<div>'  +
      '<table style="font-size: 5px; margin-bottom: 1px; margin-top: 1px; width: 100%; padding: 0; border-bottom-style: solid; border-bottom-width: 0.1px; border-bottom-color: ' + primary_color + ';">' +
        '<tbody>' +
        '<tr>' +
          '<td style="font-weight: bold; text-align: left; width: 50%;">CARTERA DE CLIENTES</td>' +
          '<td style="color: ' + secondary_color + '; width: 50%; text-align: right;">' + get_string_full_date(new Date())  + '</td>' +
        '</tr>' +
        '<tr>' +
          '<td style="color: ' + secondary_color + '; text-align: left;" colspan="2">' + 'Gestor: ' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';">' + gestor.toUpperCase() + '</span>' + '</td>' +
        '</tr>' +
        '<tr>' +
          '<td style="color: ' + secondary_color + '; text-align: left;">' + 'Día: ' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';">' + dia.toUpperCase() + '</span>' + '</td>' +
          '<td style="color: ' + secondary_color + '; text-align: left;">' + '' + '<span style="font-weight: bold; text-align: left; color: ' + primary_color + ';">' + '</span>' + '</td>' +
          '</tr>' +
        '</tbody>' +
      '</table></div>';
    const logo = '<img style="display: block; margin-left: auto; margin-right: auto;"' +
      'width="100px" src="data:image/png;base64," />';
    const _page_styles = '<style> @page{ margin-left: 3pt; margin-right: 3pt; margin-top: 3pt; margin-bottom: 3pt; padding-left: 0pt; padding-right: 0pt; padding-top: 0pt; padding-bottom: 0pt; } </style>';
    const body = '<body>' + _page_styles + logo + title_table + _data + '</body>';
    const html = '<html>' + body + '</html>';
    return html;
  };

 
  
export const report_utils = { 
    get_html,
};
